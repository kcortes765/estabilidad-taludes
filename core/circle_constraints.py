...
class CalculadorLimites:
    ...
    def calcular_limites_desde_perfil(self, perfil_terreno: List[Tuple[float, float]], 
                                    configuracion: Optional[Dict[str, float]] = None) -> LimitesGeometricos:
        """
        ...
        if configuracion is None:
            ...
            print(f" Tipo de talud detectado: {tipo_talud_detectado} (ángulo: {angulo_talud_deg:.1f}°)")
            
            configuraciones_predefinidas = crear_limites_predefinidos()
            configuracion = configuraciones_predefinidas.get(tipo_talud_detectado, 
                                                           configuraciones_predefinidas['talud_empinado'])
        ...
        altura_maxima_centro = y_max + altura_talud * configuracion.get('factor_altura_maxima', 1.5)
        ...
        print(f"   Centro X: [{centro_x_min:.2f}, {centro_x_max:.2f}] (margen: ±{margen_x:.2f})")
        print(f"   Centro Y: [{centro_y_min:.2f}, {centro_y_max:.2f}] (altura talud + {altura_talud*0.3:.2f} a +{altura_talud*configuracion.get('factor_altura_maxima', 1.5):.2f})")
        print(f"   Radio: [{radio_min:.2f}, {radio_max:.2f}] ({altura_talud*0.8:.2f}H a {altura_talud*configuracion.get('factor_radio_max', 1.5):.2f}H)")
    ...
    def validar_y_corregir_circulo(self, 
                                  circulo: CirculoFalla,
                                  limites: LimitesGeometricos,
                                  corregir_automaticamente: bool = True) -> ResultadoValidacion:
        """
        ...
        if circulo.xc < limites.centro_x_min:
            violaciones.append(f"Centro X muy a la izquierda: {circulo.xc:.2f} < {limites.centro_x_min:.2f}")
            if corregir_automaticamente:
                xc_corregido = limites.centro_x_min
        
        if circulo.xc > limites.centro_x_max:
            violaciones.append(f"Centro X muy a la derecha: {circulo.xc:.2f} > {limites.centro_x_max:.2f}")
            if corregir_automaticamente:
                xc_corregido = limites.centro_x_max
        
        if circulo.yc < limites.centro_y_min:
            violaciones.append(f"Centro Y muy bajo: {circulo.yc:.2f} < {limites.centro_y_min:.2f}")
            if corregir_automaticamente:
                yc_corregido = limites.centro_y_min
        
        if circulo.yc > limites.centro_y_max:
            violaciones.append(f"Centro Y muy alto: {circulo.yc:.2f} > {limites.centro_y_max:.2f}")
            if corregir_automaticamente:
                yc_corregido = limites.centro_y_max
        
        # Validar límites del radio
        if circulo.radio < limites.radio_min:
            violaciones.append(f"Radio muy pequeño: {circulo.radio:.2f} < {limites.radio_min:.2f}")
            if corregir_automaticamente:
                radio_corregido = limites.radio_min
        
        if circulo.radio > limites.radio_max:
            violaciones.append(f"Radio muy grande: {circulo.radio:.2f} > {limites.radio_max:.2f}")
            if corregir_automaticamente:
                radio_corregido = limites.radio_max
        
        # Generar sugerencias
        if violaciones:
            sugerencias.append("Ajustar parámetros del círculo dentro de los límites permitidos")
            if circulo.radio < limites.radio_min:
                sugerencias.append(f"Aumentar radio a al menos {limites.radio_min:.2f}")
            if circulo.yc < limites.centro_y_min:
                sugerencias.append(f"Subir centro a al menos Y={limites.centro_y_min:.2f}")
        ...
        logging.debug(f"Resultado validación: válido={es_valido}, violaciones={violaciones}, circulo_corregido={circulo_corregido}")
    ...
    def _corregir_circulo(self, circulo: CirculoFalla, limites: LimitesGeometricos) -> CirculoFalla:
        ...
        logging.debug(f"Círculo corregido de X={circulo.xc:.2f},Y={circulo.yc:.2f},R={circulo.radio:.2f} a X={nuevo_cx:.2f},Y={nuevo_cy:.2f},R={nuevo_r:.2f}")
        
        return CirculoFalla(xc=nuevo_cx, yc=nuevo_cy, radio=nuevo_r)
    
    def generar_circulos_dentro_limites(self, 
                                      limites: LimitesGeometricos,
                                      cantidad: int = 10,
                                      distribucion: str = "uniforme") -> List[CirculoFalla]:
        """
        ...
        for _ in range(cantidad):
            if distribucion == "uniforme":
                ...
                cx = random.uniform(limites.centro_x_min * 0.7, limites.centro_x_max * 0.7)
                cy = random.uniform(limites.centro_y_min, limites.centro_y_min + 
...
def aplicar_limites_inteligentes(perfil_terreno: List[Tuple[float, float]], 
                               tipo_talud: str = "talud_empinado",
                               factor_seguridad_objetivo: float = 1.5) -> LimitesGeometricos:
    """
...
def validar_circulo_con_limites(circulo: CirculoFalla,
                              perfil_terreno: List[Tuple[float, float]],
                              tipo_talud: str = "talud_empinado") -> ResultadoValidacion:
    """Validación rápida de círculo con límites automáticos"""
    
    print(f" Validando círculo X={circulo.xc:.2f}, Y={circulo.yc:.2f}, R={circulo.radio:.2f} para {tipo_talud}")
    ...
    calculador = CalculadorLimites()
    resultado = calculador.validar_y_corregir_circulo(circulo, limites, corregir_automaticamente=True)
    
    print(f" Resultado validación: válido={resultado.es_valido}")
    if resultado.circulo_corregido:
        print(f" Círculo CORREGIDO de X={circulo.xc:.2f},Y={circulo.yc:.2f},R={circulo.radio:.2f}")
        print(f"                     a X={resultado.circulo_corregido.xc:.2f},Y={resultado.circulo_corregido.yc:.2f},R={resultado.circulo_corregido.radio:.2f}")
